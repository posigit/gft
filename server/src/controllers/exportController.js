...
const exportToPDF = asyncHandler(async (req, res) => {
  ...
  res.setHeader(
    "Content-Disposition",
    "attachment; filename=feedback_export.pdf"
  );
  ...
  feedbackData.forEach((feedback, index) => {
    ...
    doc
      .fontSize(12)
      .text(`Hotel: ${feedback.hotel ? feedback.hotel.name : ""}`);
    ...
    if (feedback.assignedTo) {
      doc
        .fontSize(12)
        .text(
          `Assigned To: ${feedback.assignedTo.firstName} ${feedback.assignedTo.lastName}`
        );
    }
